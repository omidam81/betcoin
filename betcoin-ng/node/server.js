...
var responseFunc = function(req, res) {
    ...
    var indexDir = rootDir + '/build/' + argv.app + '/' + argv.loc;
        ...
        send(req, requestPath).root(indexDir).on('error', function() {
            ...
            send(req, requestPath).root(indexDir).on('error', function() {
                ...
                if ((/.html$/).test(requestPath)) {
                    ...
                    res.statusCode = 404;
                    logger.error('404 - %s', path.resolve(indexDir, requestPath));
                    res.end('not found');
                }
                logger.info('failed, trying %s', path.resolve(indexDir,requestPath));
                send(req, requestPath).root(indexDir).on('error', function() {
