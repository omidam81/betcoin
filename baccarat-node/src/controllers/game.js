...
module.exports = function(Baccarat, logger, io, betLimits, PlayerInterface) {

    var BaccaratController = function() {

    };

    BaccaratController.prototype.read = function(req, res) {
        var id = req.params.id;
        logger.debug('BaccaratController#read');
        if (id) {
            Baccarat.find(id, function(err, game) {
            ...
            Baccarat.all(query, {sort: sort, limit: limit}, function(err, games) {
    ...
    BaccaratController.prototype.next = function(req, res) {
        logger.debug('BaccaratController#next');
        Baccarat.init(function(err, newBaccarat) {
            if (err) {
                logger.error(err);
                return err.send(res);
            }
            res.json(newBaccarat);
        });
    };
    ...
    var getGameInterval = function(playerId, cb) {
        Baccarat.query().find({player_id: playerId}).sort({createdAt: -1}).limit(1).exec(function(err, lastBaccarat) {
            if (err) return cb(err);
            lastBaccarat = lastBaccarat[0];
            if (lastBaccarat) {
                var refDate = new Date();
                return cb(undefined, refDate - lastBaccarat.createdAt());
            } else {
    ...
    var playGame = function(gameData, cb) {
        gameData.play(this, function(err, gameResult) {
            if (err) return cb(err);
            io.send(gameResult.player_id(), 'player baccarat added', gameResult);
            io.excludeRoom(gameResult.player_id(), 'baccarat added', gameResult);
            cb(undefined, gameResult);
        });
    };

    BaccaratController.prototype.play = function(req, res) {
        var gameParams = {
    ...
    BaccaratController.prototype.leaderboard = function(req, res) {
        // only pull records from the last 7 days
        var cutoff = new Date(new Date() - (7 * 24 * 60 * 60 * 1000));
        Baccarat.aggregate(true).match({
