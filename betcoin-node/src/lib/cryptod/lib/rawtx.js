...
module.exports = function(cryptod, logger) {
     ...
     */
    var RawTx = function(params, callbacks) {
        ...
        this.cbs = {
            ...
            'already spent': function() {
                ...
                var err = new Error("Transaction attempted to double spend");
                err.code = RawTx.FAIL_ALREADY_SPENT;
                this.emit('failed', err);
            },
            'insufficient funds': function() {
                logger.error('insufficient funds with %d confirmations', this.minconf);
                // var err = new Error("Insufficient funds with " + this.minconf + " confirmations");
                var err = new Error("");
                err.code = RawTx.FAIL_INSUFFICIENT_FUNDS;
                this.emit('failed', err);
            },
            'empty inputs': function() {
                logger.error('empty inputs passed, use forceEmptyInputs to override');
                var err = new Error("You must specify 'forceEmptyInputs' to use an empty array of inputs");
                err.code = RawTx.FAIL_EMPTY_INPUTS;
                this.emit('failed', err);
            },
            'invalid inputs': function() {
                logger.error('invalid inputs passed, could not sign tx');
                var err = new Error("Invalid inputs used, could not sign");
                err.code = RawTx.FAIL_INVALID_INPUTS;
                this.emit('failed', err);
            }
        };
        var errs = [];
        ...
        this.failedFunc = function() {
            ...
            if (params.txfee === undefined) {
                this.txfee = cryptod.DEFAULT_TX_FEE;
        ...
        this.setTimer = function() {
            ...
            this.timeout = setTimeout(function() {
                var err = new Error("The transaction timed out");
                err.code = RawTx.FAIL_TIMEOUT;
        ...
        this.start = this.checkRefTxs = function() {
            ...
            this.setupCallbacks();
            this.setTimer();
            logger.verbose('sending %d to %j', this.amount.toBitcoin(), this.outputs, {});
            ...
            var responseFunc = function(err, unspent) {
                if (err) {
                    ...
                    logger.verbose('provided inputs: %s', JSON.stringify(self.inputs));
        ...
        this.getViableOutputs = function(cb) {
            ...
            cryptod.listUnspent(this.minconf, 9999999, this.selectFrom, function(err, unspent) {
                ...
                logger.verbose('choosing from %d outputs belonging to %s', unspent.length, self.selectFrom, {});
                ...
                while (!suitableFound && threshold < 100) {
                    ...
                    if (thisResult.qualified.length && thisResult.subTotal >= (self.amountNeeded + MIN_OUTPUT_SIZE)) {
                        logger.verbose('found %d outputs totaling %d', thisResult.qualified.length, thisResult.subTotal.toBitcoin());
                        suitableFound = true;
            ...
            if (this.amountNeeded > (-1 * MIN_OUTPUT_SIZE) && this.amountNeeded !== 0) {
                this.getViableOutputs(function(err, unspent) {
                    if (err) {
                        ...
                        if (unspent.length > 200) {
                            return self.emit('failed', new Error("Too many inputs found, please try again later"));
                        }
                        unspent.forEach(function(availableOutput) {
        ...
        this.prepareRawTx = function() {
            ...
            logger.verbose('inputs: %s, outputs: %s, amount needed: %d', JSON.stringify(this.inputs), JSON.stringify(this.outputs), this.amountNeeded.toBitcoin());
            if (this.amountNeeded <= 0) {
                ...
                logger.verbose('generated outputs', this.outputs);
            ...
            cryptod.signRawTransaction(this.rawTx, function(err, signedTx) {
                if (err) {
                    ...
                    if (signedTx.complete) {
                        ...
                        self.hex = signedTx.hex;
                        logger.verbose('signed');
                        clearTimeout(self.timeout);
    ...
    RawTx.FAIL_ALREADY_SPENT = 1;
    RawTx.FAIL_INSUFFICIENT_FUNDS = 2;
    RawTx.FAIL_TIMEOUT = 3;
    RawTx.FAIL_BITCOIND_ERROR = 4;
    RawTx.FAIL_EMPTY_INPUTS = 5;
    RawTx.FAIL_INVALID_INPUTS = 6;

    util.inherits(RawTx, events.EventEmitter);
