...
GameLogic.prototype.rankForPair = function(retValue) {
...
GameLogic.prototype.chekForStraightFlushSpecialCase = chekForStraightFlushSpecialCase;
/**
 ...
 * @returns {boolean}
 */
GameLogic.prototype.isDealerQualifiedCards = function(dealerRankObj) {
 ...
 */
GameLogic.prototype.getResult = function(dealerHand, playerHand, ante, isRaise){
    ...
    var playerFinalCards = playerHand.finalCards;
    ...
    if (isRaise) {
        ...
        if (!dealerQualified) {
            payout = ante + ante * 2 + ante; //win even money on ante and the raise will push
        } else {
            if (isWin) {
                var payout_multiplier = payout_multipliers[playerHand.rnk];
                payout = ante + ante * 2 + ante + (ante * 2) * payout_multiplier; //the ante will pay even money and the raise according to Raise Pay Table
            } else if (isPush) {
                payout = ante + ante * 2;
            }
        }
 ...
 * @returns {Array}
 */
GameLogic.prototype.getPlayerFinalCards = function(playerHand, holds, allCards) {
    var countCanceled = 0;
    var finalCards = playerHand.initCards.slice();
    for(var index in holds) {
        if (!holds[index] && finalCards[index]) {
            finalCards[index] = allCards[10 + countCanceled];
            countCanceled++;
        }
    }

    var retPlayerHand = this.getCardResult(this.parseHandCards(finalCards));
    retPlayerHand.sub_rnk = this.getGameSubRank(retPlayerHand);
    retPlayerHand.initCards = playerHand.initCards;
    retPlayerHand.finalCards = finalCards;

    return retPlayerHand;
};
