...
module.exports = function(Tigerdragon, GameLogic, logger, io, HTTPError, BaseGameController) {

    var TigerdragonController = function() {
        BaseGameController.call(this, Tigerdragon, LAST_GAME_MIN_TIME);
    };
    util.inherits(TigerdragonController, BaseGameController);
    ...
    TigerdragonController.prototype.readGameParams = function(req) {
        var gameParams = {};
        try {
    ...
    TigerdragonController.prototype.getHouseEdge = function(gameParams) {
        return GameLogic.getHouseEdge(gameParams.bets);
    };

    // override to send a socket.io broadcast when a game is finished
    TigerdragonController.prototype.playFinished = function(game, cb) {
        io.playerEmit(game.player_id(), 'player tigerdragon added', game.filter());
        io.playerBroadcast(game.player_id(), 'tigerdragon added', game.filter());
        return cb();
    };

    return TigerdragonController;
};
