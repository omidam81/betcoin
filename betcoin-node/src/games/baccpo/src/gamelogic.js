...
module.exports = function() {
            ...
            if (card1.rank === card2.rank && card2.rank === card3.rank) {
                if (card1.rank === '3') {
                    if (card1.suit === card2.suit && card2.suit === card3.suit) {
                        if (card1.suit === 'S') {
                            return g_rankOrder.three_three_of_spades;
                        } else {
                            return g_rankOrder.three_suited_threes;
                        }
                    }
                    return g_rankOrder.three_unsuited_threes;
                }
                if (card1.suit === card2.suit && card2.suit === card3.suit) {
                    return g_rankOrder.suited_three_of_a_kind;
                }
                if (card1_point === 0 && card2_point === 0 && card3_point === 0) {
                    if (card1.suit === card2.suit && card2.suit === card3.suit) {
                        return g_rankOrder.three_suited_0_point_cards;
                    }
                }
                return g_rankOrder.unsuited_three_of_a_kind;
            }
            if (card1_point === 0 && card2_point === 0 && card3_point === 0) {
                if (card1.suit === card2.suit && card2.suit === card3.suit) {
                    return g_rankOrder.three_suited_0_point_cards;
                } else {
                    return g_rankOrder.three_unsuited_0_point_cards;
                }
            }

            if (card1.suit === card2.suit && card2.suit === card3.suit) {
                return g_rankOrder.three_card_flush;
            }
        }
        return g_rankOrder.all_other;
    };
    ...
    GameLogic.prototype.getPoint = function(cards) {
        var total = 0;
    ...
    GameLogic.prototype.getHandResultText = function(dealerHand, playerHand) {
        var self = this;
        if (playerHand.rank <= g_rankOrder.three_unsuited_threes) {
            return 'player';
        }
        dealerHand.point = self.getPoint(dealerHand.finalCards);
        playerHand.point = self.getPoint(playerHand.finalCards);
        if (dealerHand.point > playerHand.point) {
            return 'dealer';
     ...
     */
    GameLogic.prototype.getPayouts = function(bets, isWin, isPush, playerHand) {
        var self = this;
        ...
        if (bets.bonus && bonus_payout_multipliers[playerHand.rank] > 0) {
            payouts.bonus = (1 + bonus_payout_multipliers[playerHand.rank]) * bets.bonus;
        }
     ...
     */
    GameLogic.prototype.getResult = function(dealerHand, playerHand, bets, action, allCards){
        var self = this;
        ...
        var result = self.getHandResultText(dealerHand, playerHand);
        ...
        var payouts = self.getPayouts(bets, isWin, isPush, playerHand);
