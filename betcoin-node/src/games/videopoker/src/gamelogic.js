 ...
 * @returns {number}
 */
GameLogic.prototype.getPayout = function(playerHandRank, wager) {
 ...
 * @returns {Array}
 */
GameLogic.prototype.getPlayerFinalCards = function(playerInitCards, holds, allCards) {
    var countCanceled = 0;
 ...
 * @returns {boolean}
 */
GameLogic.prototype.isJacksOrHigher = function(playerHandRankObj) {
 ...
 */
GameLogic.prototype.getResult = function(playerHand, wager, holds, allCards){
...
GameLogic.prototype.getAutoHolds = function(playerCards, playerHand) {
    var holds = [false, false, false, false, false];
    var i, j;
    if (this.isJacksOrHigher(playerHand)) {
        holds = [true, true, true, true, true];
        for(i = 0; i < 5; i++) {
            if (playerHand.restofData && playerHand.restofData.indexOf(this.getValue(playerCards[i].rank) + 2) !== -1) {
                holds[i] = false;
            }
        }
        return holds;
