        ...
        .end(function(err, res) {
            ...
            request.post('/' + GAME)
                ...
                .expect(function(res) {
                    ...
                    assert.equal(res.body.dealer_hand.initCards.length, 1);
                })
                .end(function(err, res) {
                    if (err) return cb(err);
                    ...
                    request.put('/' + GAME)
                        ...
                        .expect(function(res) {
                            ...
                            Wallet.get(wallet.primary(), function(err, _wallet) {
                                if (err) return cb(err,res);
                                var newBalance = INITIAL_BALANCE - 3000 - 6000 + res.body.winnings;
                                if (newBalance !== _wallet.balance()) {
                                    var errString = format("Balance mismatch after gameplay %d !== %d",
                                        newBalance,
                                        _wallet.balance());
    ...
    describe(GAME, function() {
        ...
        describe('POST /' + GAME, function() {
            ...
            it('should not play a game with limited balance', function (done) {
                Wallet.db.update({_id: wallet.primary()}, {$set: {balance: 5000, availableBalance: 5000}}, function() {});
                request.post('/' + GAME)
                    .set('X-Currency', COIN)
                    .set('Authorization', 'Bearer ' + user.token())
                    .send({
