        ...
        .end(function(err, res) {
            ...
            request.post('/' + GAME)
                ...
                .send({
                    ...
                    bets: "{\"ante\":1000, \"pairplus\":2000}"
                })
                .expect(200)
                .expect(function(res) {
                    assert.ok(res.body.player_hand);
                    assert.ok(res.body.dealer_hand === undefined);
                })
                .end(function(err, res) {
                    if (err) return cb(err);
                    ...
                    request.put('/' + GAME)
                        ...
                        .expect(function(res) {
                            ...
                            Wallet.get(wallet.primary(), function(err, _wallet) {
                                if (err) return cb(err,res);
                                var newBalance = INITIAL_BALANCE - 3000 - 1000 + res.body.winnings;
                                if (newBalance !== _wallet.balance()) {
                                    var errString = format("Balance mismatch after gameplay %d !== %d",
                                        newBalance,
                                        _wallet.balance());
...
module.exports = function(COIN) {
    ...
    describe(GAME, function() {
        ...
        describe('GET /' + GAME + '/next', function() {
            ...
            it('should should be accessable from the mountpath \'/3card\'', function(done) {
                request.get('/3card/next')
                    .set('X-Currency', COIN)
                    .set('Authorization', 'Bearer ' + user.token())
                    .expect(200)
                    .expect(function(res) {
        ...
        describe('POST /' + GAME, function() {
            ...
            it('should play a game', function(done) {
                request.post('/' + GAME)
                    ...
                    .expect(function(res) {
                        assert.ok(res.body.player_hand);
                        assert.equal(res.body.dealer_hand, undefined);
                    })
                    .end(function(err, res) {
                        if (err) return done(err);
                        Wallet.get(wallet.primary(), function(err, _wallet) {
            ...
            it('should reject a missing bets', function(done) {
                request.post('/' + GAME)
                    .set('X-Currency', COIN)
                    .set('Authorization', 'Bearer ' + user.token())
                    .send({
            ...
            it('should not play a game with limited balance', function (done) {
                Wallet.db.update({_id: wallet.primary()}, {$set: {balance: 3500, availableBalance: 3500}}, function() {});
                request.post('/' + GAME)
                    .set('X-Currency', COIN)
                    .set('Authorization', 'Bearer ' + user.token())
                    .send({
                    ...
                    .expect(412)
                    .expect(function(res) {
                        assert.equal(res.body.message, "Limited Balance");
                    })
                    .end(done);
            });
