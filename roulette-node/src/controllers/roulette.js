...
module.exports = function(Roulette, logger, io, betLimits, PlayerInterface) {

    var RouletteController = function() {

    };

    RouletteController.prototype.read = function(req, res) {
        var id = req.params.id;
        logger.debug('RouletteController#read');
        if (id) {
            Roulette.find(id, function(err, game) {
            ...
            Roulette.all(query, {sort: sort, limit: limit}, function(err, games) {
    ...
    RouletteController.prototype.next = function(req, res) {
        logger.debug('RouletteController#next');
        Roulette.init(function(err, newRoulette) {
            if (err) {
                logger.error(err);
                return err.send(res);
            }
            res.json(newRoulette);
        });
    };
    ...
    var getGameInterval = function(playerId, cb) {
        Roulette.query().find({player_id: playerId}).sort({createdAt: -1}).limit(1).exec(function(err, lastRoulette) {
            if (err) return cb(err);
            lastRoulette = lastRoulette[0];
            if (lastRoulette) {
                var refDate = new Date();
                return cb(undefined, refDate - lastRoulette.createdAt());
            } else {
    ...
    var checkPlayerToken = function(cb) {
        ...
        PlayerInterface.verifyToken(this.player_id, this.token, function(err, isValid) {
            ...
            // the waterfall is Roulette.checkLock which consumes a
            // gameId string
    ...
    var playGame = function(gameData, cb) {
        gameData.play(this, function(err, gameResult) {
            if (err) return cb(err);
            io.send(gameResult.player_id(), 'player roulette added', gameResult);
            io.excludeRoom(gameResult.player_id(), 'roulette added', gameResult);
            cb(undefined, gameResult);
        });
    };


    RouletteController.prototype.play = function(req, res) {
        var gameParams = {
    ...
    RouletteController.prototype.leaderboard = function(req, res) {
        // only pull records from the last 7 days
        var cutoff = new Date(new Date() - (7 * 24 * 60 * 60 * 1000));
        Roulette.aggregate(true).match({
