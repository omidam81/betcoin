(function(window, angular) {
    ...
    var appConfig = function($routeProvider, $locationProvider, $compileProvider, BCPlayerProvider, PlayerApi, SocketServer) {
        ...
        $routeProvider.when('/', {
            ...
            controller: 'LotteryGameDetailsController'
        }).when('/home', {
            templateUrl: 'tpl/history.html',
            controller: 'HistoryController'
        }).when('/history/:type?', {
    ...
    var appRun = function($rootScope, GameSocket) {
        ...
        GameSocket.on('player lotterybet added', function(game) {
            setTimeout(function() {
        ...
        GameSocket.on('lotterybet added', function(game) {
            $rootScope.$broadcast('global game added', game);
        });

        var processWinner = function(winner){
            $rootScope.$broadcast('winner selected', winner);
        };

        GameSocket.on('player winner selected', function(data){
            processWinner(data);
        });
        GameSocket.on('winner selected', function(data){
            processWinner(data);
        });
    };
