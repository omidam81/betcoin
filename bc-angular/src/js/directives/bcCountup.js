(function(Application) {
	...
	function update() {
		...
		while( (scope = tstak.pop()) ) {
			...
			if(delta < 0 || scope.diff <= 0 || isNaN(delta)) {
				scope.curVal = scope.tgtVal;
				scope.isCounting = false;
			} else {
                // console.debug(scope.curVal, scope.tgtVal, delta, EXP_EASE, scope.diff);
	...
	Application.Directives.directive('bcCountup', [
		'$filter',
		function($filter) {
			return {
				replace: true,
