...
module.exports = function() {
    ...
    var Tile = function(obj) {
        ...
        this.rank = function() {
            for (var i = 0; i < self.unshuffledTiles.length; i++) {
                if (equal(tileSelf, self.unshuffledTiles[i])) {
                    return self.unshuffledTiles[i].rank;
                }
            }
            return 0;
        };
    ...
    // for unit test
    this.getHand = function(objs) {
        return new Hand(objs);
    };
    this.getTile = function(obj) {
        return new Tile(obj);
    };
    this.getGroup = function(objs) {
        var tiles = [];
        for(var i = 0; i < objs.length; i++) {
            tiles.push(new Tile(objs[i]));
        }
