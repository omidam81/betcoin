...
module.exports = function(modelStore, GameLogic, PlayerInterface, logger) {
    ...
    var Game = modella('minesweeper')
            ...
            .attr('columns', {required: true, type: 'number'}) // number of columns in game board
            .attr('rows', {required: true, type: 'number'}) // number of rows in game board
    ...
    Game.prototype.play = function(params, cb) {
        ...
        var playGame = function(player, done) {
            ...
            logger.debug('##### playing game %s for %s (%s) in #####', self.primary(), player.alias, player_id);
            logger.debug('result');
