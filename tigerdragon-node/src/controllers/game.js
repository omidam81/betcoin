...
module.exports = function(Tigerdragon, logger, io, betLimits, PlayerInterface) {

    var TigerdragonController = function() {

    };

    TigerdragonController.prototype.read = function(req, res) {
        var id = req.params.id;
        logger.debug('TigerdragonController#read');
        if (id) {
            Tigerdragon.find(id, function(err, game) {
            ...
            Tigerdragon.all(query, {sort: sort, limit: limit}, function(err, games) {
    ...
    TigerdragonController.prototype.next = function(req, res) {
        logger.debug('TigerdragonController#next');
        Tigerdragon.init(function(err, newTigerdragon) {
            if (err) {
                logger.error(err);
                return err.send(res);
            }
            res.json(newTigerdragon);
        });
    };
    ...
    var getGameInterval = function(playerId, cb) {
        Tigerdragon.query().find({player_id: playerId}).sort({createdAt: -1}).limit(1).exec(function(err, lastTigerdragon) {
            if (err) return cb(err);
            lastTigerdragon = lastTigerdragon[0];
            if (lastTigerdragon) {
                var refDate = new Date();
                return cb(undefined, refDate - lastTigerdragon.createdAt());
            } else {
    ...
    var checkPlayerToken = function(cb) {
        ...
        PlayerInterface.verifyToken(this.player_id, this.token, function(err, isValid) {
            ...
            // the waterfall is Tigerdragon.checkLock which consumes a
            // gameId string
    ...
    var playGame = function(gameData, cb) {
        gameData.play(this, function(err, gameResult) {
            if (err) return cb(err);
            io.send(gameResult.player_id(), 'player tigerdragon added', gameResult);
            io.excludeRoom(gameResult.player_id(), 'tigerdragon added', gameResult);
            cb(undefined, gameResult);
        });
    };

    TigerdragonController.prototype.play = function(req, res) {
        var gameParams = {
    ...
    TigerdragonController.prototype.leaderboard = function(req, res) {
        // only pull records from the last 7 days
        var cutoff = new Date(new Date() - (7 * 24 * 60 * 60 * 1000));
        Tigerdragon.aggregate(true).match({
    ...
    return new TigerdragonController();
};
