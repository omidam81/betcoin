...
module.exports = function(GameModel, logger, io, betLimits, PlayerInterface) {
    ...
    GameController.prototype.read = function(req, res) {
        ...
        if (id) {
            GameModel.find(id, function(err, game) {
            ...
            GameModel.all(query, {sort: sort, limit: limit}, function(err, games) {
    ...
    GameController.prototype.next = function(req, res) {
        ...
        GameModel.init(function(err, newGame) {
            if (err) {
                logger.error(err);
                return err.send(res);
            }
    ...
    var getGameInterval = function(playerId, cb) {
        GameModel.query().find({player_id: playerId}).sort({createdAt: -1}).limit(1).exec(function(err, lastGameModel) {
            if (err) return cb(err);
            lastGameModel = lastGameModel[0];
            if (lastGameModel) {
                var refDate = new Date();
                return cb(undefined, refDate - lastGameModel.createdAt());
            } else {
    ...
    var checkPlayerToken = function(cb) {
        ...
        PlayerInterface.verifyToken(this.player_id, this.token, function(err, isValid) {
            ...
            // the waterfall is GameModel.checkLock which consumes a
            // gameId string
    ...
    var playGame = function(gameData, cb) {
        gameData.play(this, function(err, gameResult) {
            ...
            if(gameResult.status() === 'finished'){
                io.send(gameResult.player_id(), 'player jump added', gameResult);
                io.excludeRoom(gameResult.player_id(), 'jump added', gameResult);
            }
    ...
    GameController.prototype.leaderboard = function(req, res) {
        ...
        GameModel.aggregate(true).match({
