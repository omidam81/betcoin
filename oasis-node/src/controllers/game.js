...
module.exports = function(Game, logger, io, betLimits, PlayerInterface) {
    ...
    var doNextAction = function(gameData, cb) {
        gameData.nextAction(this, function(err, gameResult) {
            if (err) return cb(err);
            io.send(gameResult.player_id(), 'player oasis added', gameResult);
            io.excludeRoom(gameResult.player_id(), 'oasis added', gameResult);
            cb(undefined, gameResult);
        });
    };
    ...
    var doSwitchAction = function(gameData, cb) {
        gameData.switchAction(this, function(err, gameResult) {
            if (err) return cb(err);
            return cb(undefined,gameResult);
        });
    };    
    GameController.prototype.switchAction = function(req, res){
        var gameParams = {
            gameId: req.body._id||req.body.game_id,
            player_id: req.body.player_id,
            token: req.token,
        ...
        try{
            ...
            logger.debug('parse holds json error', e);
            return new HTTPError(400, "invalid holds json string", res);
        }
        ...
        async.waterfall([
            ...
            // actually play the game, supplies the finished game data
            // to the callback function
            doSwitchAction.bind(gameParams)
        ], function(err, result) {
            if (err) return res.send(err);
            result = result.filter();
            delete result.final_array;
