...
var DataDefinitionsService = require('./data-definitions-service');

var QueryGenerator = function(mongo) {
    var dataDefinitionsService = new DataDefinitionsService();
    var generator = this;
    ...
    generator.resolveDataDefinitionDependency = function(query){
        query.datapoints.forEach(function(datapoint){
            var dataDefinition = dataDefinitionsService.getDataDefinition(datapoint.name);
    ...
    generator.resolveAggregateTask = function(query) {
        ...
        query.datapoints.forEach(function(datapoint) {
            ...
            if (DataDefinition.groupBy) {
                try{
                    var groupbyDataDefinition = dataDefinitionsService.getDataDefinition(DataDefinition.groupBy);
                    project[DataDefinition.groupBy] = groupbyDataDefinition.format;
                }catch(ex){
                    project[DataDefinition.groupBy] = 1;
                }

                var existingDataDefinition = dataDefinitionsService.getDataDefinition(DataDefinition.field);
        ...
        if (query.sort) {
            ...
            for (var i in query.datapoints) {
                if (query.datapoints[i].name === query.sort.datapoint) {
                    sortDataDefinition = dataDefinitionsService.getDataDefinition(query.sort.datapoint);
                    break;
                }
            }
        ...
        var resolvedCollectionNames = dataDefinitionsService.getCollectionNames(query.datapoints);
    ...
    generator.createResultMatchCriteria = function(aggregateTask, dataQueue) {
        ...
        aggregateTask.dataQuery.datapoints.forEach(function(datapoint){
            if(datapoint.result_match && dataQueue.length > 0){
                ...
                }, dataDefinitionsService.getDataDefinition(datapoint.name), match);
            }
        });
        if(match){
            match = {$match: match};
                    ...
                    db[aggregateTask.collection].aggregate(aggregateTask.pipeline, function(err, _results){
                        _results.forEach(function(result){
                            ...
                            Object.keys(result).forEach(function(key){
                                var dataDefinition = dataDefinitionsService.getDataDefinition(key);
