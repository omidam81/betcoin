...
var DataDefinitionService = function() {
    var self = this;

    this.getAllDataDefinitions = function() {
        var definitions = [],
            hashcheck = {};
    ...
    this.getDataDefinition = function(name) {
        for (var i in self.CollectionDefinitions) {
            if (self.CollectionDefinitions.hasOwnProperty(i)) {
                for (var j in self.CollectionDefinitions[i].datapoints) {
                    if (self.CollectionDefinitions[i].datapoints.hasOwnProperty(j)) {
    ...
    this.getCollectionNames = function(datapoints) {
        var collectionNames = {};
        datapoints.forEach(function(datapoint) {
            var DataDefinition = self.getDataDefinition(datapoint.name);
            collectionNames[DataDefinition.collection] = 1;
        });
        var nameList = [];
...
module.exports = DataDefinitionService;
