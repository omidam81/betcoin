...
module.exports = function(Game, logger, io, betLimits, PlayerInterface) {
    ...
    var getGameInterval = function(playerId, cb) {
        logger.debug('GameController#next.getGameInterval');
    ...
    var checkPlayerToken = function(cb) {
        logger.debug('GameController#next.checkPlayerToken');
        var self = this;
        PlayerInterface.verifyToken(this.player_id, this.token, function(err, isValid) {
            ...
            // the waterfall is War.checkLock which consumes a
            // gameId string
    ...
    var checkLimits = function(gameData, cb) {
        logger.debug('GameController#next.checkLimits');
        var self = this;
    ...
    var checkInterval = function(gameData, cb) {
        logger.debug('GameController#next.checkInterval');
    ...
    var playGame = function(gameData, cb) {
        logger.debug('GameController#next.playGame');
        gameData.play(this, function(err, gameResult) {
            if (err) return cb(err);
            io.send(gameResult.player_id(), 'player craps added', gameResult);
            io.excludeRoom(gameResult.player_id(), 'craps added', gameResult);
            cb(undefined, gameResult);
        });
    };
    var doNextAction = function(gameData, cb) {
        logger.debug('GameController#next.doNextAction');
    ...
    var returnBets = function(gameData, cb) {
        logger.debug('GameController#next.returnBets');
        gameData.returnBets(this, function(err, gameResult) {
            if (err) return cb(err);
            cb(undefined, gameResult);
        });
    };
    ...
    GameController.prototype.play = function(req, res) {
        logger.debug('GameController.play');
        var gameParams = {
    ...
    GameController.prototype.nextAction = function(req, res) {
        logger.debug('GameController.nextAction');
        var gameParams = {
        ...
        async.waterfall([
            ...
            if (err) return res.send(err.code, err);
            result = result.filter();
            result.last = result.round_history[result.round_history.length - 1];
            delete result.round_history;
            res.json(result);
        });
    };

    GameController.prototype.returnBets = function(req, res) {
        logger.debug('GameController.nextAction');
        var gameParams = {
            gameId: req.body._id||req.body.game_id,
            lastGameId: req.body.last_game_id,
        ...
        try {
            ...
            return new HTTPError(400, "Invalid return bets param", res);
        }
        // check params from user
        if (!gameParams.gameId) {
            return new HTTPError(400, "missing game id from play request", res);
    ...
    GameController.prototype.leaderboard = function(req, res) {
        logger.debug('GameController.leaderboard');
